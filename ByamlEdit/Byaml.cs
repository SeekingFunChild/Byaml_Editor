...
namespace ByamlEdit
{
    class Byaml
    {
        ...
        List<ByamlNode> changed_nodes;
        List<TreeNode> searched_nodes;

        //smubin修复
        List<KeyValuePair<int, TreeNode>> Parameters;
        List<KeyValuePair<int, TreeNode>> Translates;
        List<KeyValuePair<int, TreeNode>> Rotates;
        // List<int> paras; //
        // List<string> units;    //
        ...
        public Byaml(string byamlFile)
        {
           ...
           values = new List<string>();
            ...
            Parameters = new List<KeyValuePair<int, TreeNode>>();
            Translates = new List<KeyValuePair<int, TreeNode>>();
            Rotates = new List<KeyValuePair<int, TreeNode>>();

            changed_nodes = new List<ByamlNode>();
        ...
        private void SaveChanged(string text,ByamlNode byamlNode)
        {
            switch (byamlNode.Type)
            {
                case ByamlNodeType.String:
                    if (!changed_nodes.Contains(byamlNode))
                    {
                        changed_nodes.Add(byamlNode);
                    }
                    int value = values.IndexOf(text);
                    if(value!=-1)
                    {
                        ((ByamlNode.String)byamlNode).Value = value;
                    }
                    else
                    {
                        MessageBox.Show("not found in values");
                    }
                    break;
                case ByamlNodeType.Boolean:
                    ...
                    ((ByamlNode.Boolean)byamlNode).Value=bool.Parse(text);
                    break;
                case ByamlNodeType.UInt:
                    ...
                    ((ByamlNode.Int)byamlNode).Value = UInt32.Parse(text);
                    break;
                case ByamlNodeType.Hash:
                    ...
                    ((ByamlNode.Hash)byamlNode).Value = UInt32.Parse(text);
                    break;
                case ByamlNodeType.Single:
                    ...
                    ((ByamlNode.Single)byamlNode).Value = float.Parse(text);
                    break;
        ...
        public void Show(Control control)
        {
            ...
            Save = new Button();
            Save.Text = "Save";
            Save.Top += treeView.Top + treeView.Height + 10;
            Save.Left = treeView.Right-Save.Width;
            Save.Click += SaveClick;
            control.Controls.Add(Save);

            SearchText = new TextBox();
            SearchText.Top = Save.Top;
        ...
        private void WriteByamlNodes(ShowNode root,int offset,EndianBinaryWriter writer)
        {
            ...
            switch (type)
            {
                ...
                case ByamlNodeType.NamedNode:
                    ...
                    TreeNode transNode = null;
                    TreeNode RotateNode = null;
                    foreach (ShowNode node in root.Nodes)
                    {
                        if (node.Text.ToLower() == "!Parameters".ToLower())
                        {
                            hasParameters = true;
                            paraNode = node;
                        }
                        if (node.Text.ToLower() == "Translate".ToLower())
                        {
                            hasTranslate = true;
                            transNode = node;
                        }
                        if (node.Text.ToLower() == "Rotate".ToLower())
                        {
                            hasRotate = true;
                            RotateNode = node;
                        }
                    ...
                    if (hasParameters)
                    {
                        ...
                        {
                            if(TreeNodeCmp(item.Value,paraNode))
                            {
                                paraExist = true;
                                paraOffset = item.Key;
                            }
                    ...
                    if (hasTranslate)
                    {
                        ...
                        {
                            if (TreeNodeCmp(item.Value, transNode))
                            {
                                transExist = true;
                                transOffset = item.Key;
                            }
                        }
                    }
                    ...
                    if (hasRotate)
                    {
                        ...
                        {
                            if (TreeNodeCmp(item.Value,RotateNode))
                            {
                                RotateExist = true;
                                RotateOffset = item.Key;
                            }
                        }
                    }
                    ...
                    if(needCheck)
                    {
                        foreach (ShowNode node in root.Nodes)
                        {
                            if (node.Text.ToLower() == "UnitConfigName".ToLower())
                            {
                                int index = 0;
                                unitName = node.Nodes[0].Text;
                                foreach (var unit in units)
                                {
                                    int len = unit.Length > unitName.Length ? unitName.Length / 2 : unit.Length / 2;
                                    if (unit.Substring(0, len)==unitName.Substring(0, len))
                                    {
                                        paraExist = true;
                                        paraOffset = paras[index];
                                        break;
                                    }
                                    index++;
                                }
                        ...
                        switch (node.Type)
                        {
                            ...
                            case ByamlNodeType.NamedNode:
                                if(node.Text.ToLower() == "!Parameters".ToLower())
                                {
                                    ...
                                    {
                                        //写入节点的实例地址
                                        writer.Write(paraOffset);
                                        p0 = (int)writer.BaseStream.Position;
                                    }
                                    else
                                    {
                                        Parameters.Add(new KeyValuePair<int, TreeNode>(p1,node));
                                ...
                                else if (node.Text.ToLower() == "Translate".ToLower())
                                {
                                    if (transExist)
                                    {
                                        //写入节点的实例地址
                                    ...
                                    {
                                        Translates.Add(new KeyValuePair<int, TreeNode>(p1, node));
                                        //写入节点的实例地址
                                        writer.Write(p1);
                                        p0 = (int)writer.BaseStream.Position;
                                        //写入节点实例
                                ...
                                else if (node.Text.ToLower() == "Rotate".ToLower())
                                {
                                    if (RotateExist)
                                    {
                                        //写入节点的实例地址
                                    ...
                                    {
                                        Rotates.Add(new KeyValuePair<int, TreeNode>(p1, node));
                                        //写入节点的实例地址
                                        writer.Write(p1);
                                        p0 = (int)writer.BaseStream.Position;
                                        //写入节点实例
